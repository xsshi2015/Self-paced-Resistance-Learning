...
def predict_02(model, dataloader, true_labels, batch_size=50, use_gpu=True):
    ...
    for iteration, data in enumerate(dataloader, 0):
        #data = next(iter(dataloaders['val']))
        inputs, _, index = data['image'], data['labels'], data['index']
        # import pdb; pdb.set_trace()
        if inputs.size(3)==3:
            inputs = inputs.permute(0,3,1,2)
            inputs = inputs.type(torch.FloatTensor)

        labels = torch.from_numpy(true_labels[index]).type(torch.LongTensor)
        # print("Collecting values for train data[{}/{}]".format(iteration, num_train_batch-1))
...
def predict_03(model, dataloader, true_labels, batch_size=50, use_gpu=True):
    ...
    for iteration, data in enumerate(dataloader, 0):
        ...
        pred_weights, predicted = torch.max(y.data, 1)
        total += labels.size(0)
        correct += predicted.eq(labels.data).cpu().sum()

        pred_labels.extend(predicted.cpu().numpy())        
        pred_train_weights.extend(pred_weights.cpu().numpy())

    pred_labels = np.squeeze(pred_labels)
    pred_train_weights = np.squeeze(pred_train_weights)

    
    Accuracy = 100.*np.float(correct)/total

    return Accuracy, pred_labels, pred_train_weights

def predict_01(model, dataloader, batch_size=50, use_gpu=True):
