...
def generate_weight(trainLabels, predictLabels, epoch, b_epoch, s0=2, gamma_d=10, total_epochs=200):
...
def generate_data_weight(model, dataloader_test, epoch, b_epoch, s0=2, gamma_d=10, use_gpu=True):
    trainLabels = []
    predictLabels = []
    model.eval()
...
def train(model, optimizer, dataloader, test_loader,dataloader_test, true_label, noisy_rate):
    ...
    io.savemat('cifar100_symmetric_Conv_epoch_test_accuracy'+'_'+ str(noisy_rate)+'.mat', mdict={'test_accuracy': epoch_test_accuracy})
...
def main():
    ...
    for iter in range(rate.size):
        ...
        image_datasets ={'train': Cifar100(train=True, transform = transforms_train),
                      'test': Cifar100(train=False, transform = transforms_test)}
        ...
        train(model, optimizer, train_loader, test_loader, train_loader_test, true_label, rate[iter])
