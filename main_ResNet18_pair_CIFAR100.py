...
from Cifar100Dataset import Cifar100Dataset as Cifar100
...
parser.add_argument('-an', '--anchor_num', default=1000, type=int, metavar='N', help='Number of anchors (default: 100)')
...
def pre_train(dataloader, test_loader, dataloader_test, true_label, noisy_rate, checkpoint_filename, total_epochs = 100, learning_rate=1e-5, use_gpu=True, arch = 'resnet_', seed=123):
    ...
    model = ResNet(class_num=args.num_class)
    ...
    for epoch in range(start_epoch, total_epochs):      
        ...
        if epoch<b_epoch:
            ...
            if epoch==b_epoch:
                ...
                st = (total_epochs-30-b_epoch)//max(10-min(s0,5),1)
                u_max = (10-min(s0,5))*30     ## if rate=0, u_max = (10-min(s0,5))*10
            if epoch % 20==0:
    ...
    io.savemat('cifar100_pair_ResNet_epoch_test_accuracy'+'_'+ str(noisy_rate)+'.mat', mdict={'test_accuracy': epoch_test_accuracy})
...
def main():
    ...
    for iter in range(1):
        ...
        pre_train(train_loader, test_loader, train_loader_test, true_label, rate[iter], checkpoint_filename, total_epochs = 200, learning_rate=1e-3, use_gpu=True, arch='resnet_', seed=args.seed)
