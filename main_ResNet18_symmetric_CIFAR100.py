...
def masked_crossentropy(out, labels, mask_flag):
    ...
    if nbsup > 0:
        ...
        loss = F.cross_entropy(masked_outputs, masked_labels, size_average=True)
        return loss
...
def pre_train(dataloader, test_loader, dataloader_test, true_label, noisy_rate, checkpoint_filename, total_epochs = 100, learning_rate=1e-5, use_gpu=True, arch = 'resnet_', seed=123):
    ...
    io.savemat('cifar100_symmetric_ResNet_epoch_test_accuracy'+'_'+ str(noisy_rate)+'.mat', mdict={'test_accuracy': epoch_test_accuracy})
